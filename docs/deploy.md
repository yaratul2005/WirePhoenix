...
- **TLS 1.3:** We strongly recommend placing the `pw_server` daemon behind a reverse proxy like **Nginx** or **HAProxy** to terminate TLS connections. The PhoenixWire client factory supports `tls://` stream wrappers out of the box when using the pure-PHP fallback.
