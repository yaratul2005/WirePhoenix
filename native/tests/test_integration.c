...
void* server_thread(void* arg __attribute__((unused))) {
    ...
    for (int i=0; i<4; i++) { // Loop to handle tests
                ...
                while (transport.read_len > 0) {
                    ...
                    if (conn.state == PW_STATE_HANDSHAKING) {
                        ...
                        if (parsed > 0) {
                            ...
                            } else if (server_hello.resume_accepted) {
                        ...
                        size_t parsed = pw_frame_parse_header(transport.read_buffer + transport.read_pos, transport.read_len, &frame);
                        if (parsed > 0 && transport.read_len >= parsed + frame.length) {
                            if (frame.opcode == PW_OPCODE_CLOSE) {
                                transport.read_pos += parsed + frame.length;
                                transport.read_len -= (parsed + frame.length);
                            } else {
                                transport.read_pos += parsed + frame.length;
...
void do_valid_handshake() {
    ...
    close(sock);
    printf("Valid handshake test passed.\n");
}

void do_invalid_version_handshake() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(TEST_PORT);
        ...
        if (pfd.revents & POLLIN) {
            ...
            while (transport.read_len > 0) {
                ...
                if (conn.state == PW_STATE_HANDSHAKING) {
                    ...
                    size_t parsed = pw_handshake_parse_server_hello(transport.read_buffer + transport.read_pos, transport.read_len, &s_hello);
                    if (parsed > 0) {
                        assert(s_hello.status == PW_ERR_PROTOCOL_ERROR);
                        transport.read_pos += parsed; transport.read_len -= parsed;
    ...
    close(sock);
    printf("Invalid version handshake test passed.\n");
}

void do_resume_success_handshake() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(TEST_PORT);
    ...
    // Pretend this is a valid token the server recognizes
    memset(hello.resume_session_id, 0x11, PW_SESSION_ID_SIZE);
        ...
        if (pfd.revents & POLLIN) {
            ...
            while (transport.read_len > 0) {
                ...
                if (conn.state == PW_STATE_HANDSHAKING) {
                    ...
                    size_t parsed = pw_handshake_parse_server_hello(transport.read_buffer + transport.read_pos, transport.read_len, &s_hello);
                    if (parsed > 0) {
                        assert(s_hello.status == 0);
                        assert(s_hello.resume_accepted == true);
                        transport.read_pos += parsed; transport.read_len -= parsed;

                        // We skip auth and go straight to ready if resumed!
                        pw_connection_transition(&conn, PW_STATE_READY);
    ...
    close(sock);
    printf("Resume success test passed.\n");
}

void do_resume_fail_handshake() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(TEST_PORT);
    ...
    // Pretend this is an INVALID token the server REJECTS
    memset(hello.resume_session_id, 0x99, PW_SESSION_ID_SIZE);
        ...
        if (pfd.revents & POLLIN) {
            ...
            while (transport.read_len > 0) {
                ...
                if (conn.state == PW_STATE_HANDSHAKING) {
                    ...
                    if (parsed > 0) {
                        assert(s_hello.status == 0);
                        assert(s_hello.resume_accepted == false);
                        transport.read_pos += parsed; transport.read_len -= parsed;

                        // We must fallback to auth if resumed failed
                        pw_connection_transition(&conn, PW_STATE_AUTHENTICATING);

                        // Let's close instead of auth to speed up test
                        pw_frame_t close_frame = { .version = PW_VERSION_1, .flags = PW_FLAG_FIN, .opcode = PW_OPCODE_CLOSE };
                        transport.write_len += pw_frame_serialize_header(&close_frame, transport.write_buffer + transport.write_len, PW_BUFFER_SIZE - transport.write_len);
                        pw_connection_transition(&conn, PW_STATE_CLOSING);
                    } else break;
    ...
    close(sock);
    printf("Resume fallback test passed.\n");
}
...
int main() {
    ...
    do_valid_handshake();
    usleep(10000);
    do_invalid_version_handshake();
    usleep(10000);
    do_resume_success_handshake();
    usleep(10000);
    do_resume_fail_handshake();
