...
#define CHAOS_PORT 8083
#define CHAOS_ITERATIONS 20

// Thread to run a resilient server that drops connections randomly
void* chaos_server_thread(void* arg __attribute__((unused))) {
    int server_fd;
    ...
    address.sin_port = htons(CHAOS_PORT);
    ...
    for (int i=0; i<CHAOS_ITERATIONS; i++) {
        ...
        while (conn.state != PW_STATE_CLOSED && conn.state != PW_STATE_DISCONNECTED) {
            ...
            if (p <= 0) {
                tick++;
                // 10% chance to randomly sever the connection if we're active
                if (tick > 5 && (rand() % 100) < 10) {
                    break;
                }
                continue;
            }
...
void do_chaos_client_run() {
    int sock = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in serv_addr;
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_port = htons(CHAOS_PORT);
            ...
            while (transport.read_len > 0) {
                ...
                if (conn.state == PW_STATE_HANDSHAKING) {
                    ...
                    if (parsed > 0 && transport.read_len >= parsed + frame.length) {
                        ...
                        memcpy(transport.write_buffer + transport.write_len, "CHOS", 4);
...
int main() {
    srand(time(NULL));
    pthread_t thread_id;
    pthread_create(&thread_id, NULL, chaos_server_thread, NULL);
    usleep(100000);
    ...
    printf("Chaos test passed (%d iterations).\n", CHAOS_ITERATIONS);
    return 0;
}
